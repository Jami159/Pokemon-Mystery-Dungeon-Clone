...
poke = input("Enter poke: ")
action = input("Enter action: ")+"_"
d = "_"+input("Enter direction: ")+"_"
...
while bott<hi:
    ...
    while right<wid:
        ...
        if num>9:
            num = 0
            cnt += 1
        image.save(back.subsurface((left,top,right-left+1,bott-top+1)),poke+d+action+str(num)+".png")
        if double == '1':
            image.save(back.subsurface((left,top,right-left+1,bott-top+1)),BASE+"z"*(50-int(str(cnt)+str(num)))+".png")
